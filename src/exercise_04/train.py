...
def train_model(output_folder: Path, device: torch.device):
    ...
    train_loader = DataLoader(train_subset, batch_size=10, shuffle=True, pin_memory=pin_memory)
    val_loader = DataLoader(val_subset, batch_size=10, shuffle=False, pin_memory=pin_memory)

    # Define the model, loss function, and optimizer
    output_dim = 10  # CIFAR-10 has 10 classes
    model = ConvolutionalNeuralNetwork(output_dim=output_dim).to(device)
